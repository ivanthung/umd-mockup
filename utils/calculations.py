...
def create_building_profile_impact_table(building_profiles: dict, building_profiles_user_data: dict) -> pd.DataFrame:
    ...
    for profile in building_profiles_user_data.keys():
        for secondary_type in building_profiles_user_data[profile]:
            impact_table.append([profile,
                                secondary_type,
                                building_profiles_user_data[profile][secondary_type],
                                building_profiles[profile]["impact_m2"][secondary_type]*building_profiles_user_data[profile][secondary_type]])
    impact_df = pd.DataFrame(impact_table, columns=["Gebouwprofiel", "Secondary Type", "M2", "Totale impact"])        
    # Calculate the share of the imoact of the total impact of each profile 
    impact_df["Share of total impact"] = impact_df["Totale impact"] / impact_df["Totale impact"].sum() * 100
    return impact_df
    

def create_building_profile_realisation_table(building_profiles: dict,
    ...
    return pd.DataFrame(realistion_table, columns=["Gebouwprofiel", "Gebruik", "Realized m2"])

def calculate_total_amount_of_houses_per_type(m2_per_housing_type, percentage_per_housing_type, projected_houses, ) -> dict:
    ...
    for key, value in m2_per_housing_type.items():
        amount_of_houses[key] = int(value * projected_houses * percentage_per_housing_type[key] /100 )
    return amount_of_houses

def summarize_realisation_table(realistion_df: pd.DataFrame,
                                other_needs_m2: dict,
                                dwelling_needs_m2: dict,) -> pd.DataFrame:
    """
    ...
    """
    summary_df = pd.DataFrame(realistion_df.groupby("Gebruik")["Realized m2"].sum()).reset_index()
    ...
    summary_df["Percentage realized"] = summary_df["Realized m2"] / summary_df["Needed m2"] * 100
    return summary_df
