...
def render_html(text: str, figs_in_base64: dict) -> str:
    ...
    figs = [f"<h3>{fig_name}</h3> <img src='data:image/svg;base64,{fig_data}' alt='{fig_name}'>" for fig_name, fig_data in figs_in_base64.items()]
    figs_html = " ".join(figs)
    
    file_loader = FileSystemLoader('resources/templates')  # Specify the directory of your templates 
...
def generate_report_pdf(text: str, figs: dict):
    ...
    figs_in_base64 = {fig_name: transform_fig_to_base64(fig_data) for fig_name, fig_data in figs.items()}
